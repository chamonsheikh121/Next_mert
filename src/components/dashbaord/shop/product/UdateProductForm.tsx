...
export function UpdateProductForm({ product }: { product: TProduct }) {
  ...
  const form = useForm({
    defaultValues: {
      ...
      keyFeature: product?.keyFeatures?.map((value) => ({ value })) || [
        { value: "" },
      ],
      availableColors: product?.availableColors?.map((value) => ({
        value,
      })) || [{ value: "" }],
    },
  });
  ...
  async function onSubmit(data: FieldValues) {
    ...
    console.log("specifications are ", data?.specification);
  ...
  return (
    <Card className="lg:max-w-3xl mx-auto sm:max-w-md">
      <CardHeader>
        ...
        <CardDescription>
          Update product to your shop with the product details.
        </CardDescription>
      </CardHeader>
      <CardContent>
